  ...
  try {
    ...
    if (error || !data || data.length === 0) {
      ...
      EVENTS = data.map(e => ({
        id: e.id,
        title: e.title,
        category: e.categories?.name || e.category || 'Event',
...
function updateNav() {
  document.querySelectorAll('.nav-links a, .drawer-nav a[data-page]').forEach(a => {
...
async function doSignup(e) {
  ...
  if (error) showToast(error.message, 'error');
  else { showToast('Account created! Check your email to confirm.'); closeLogin(); }
}
...
function posterColors(id) {
  ...
  const idx = typeof id === 'number' ? (id - 1) % p.length : Math.abs(id.toString().charCodeAt(0)) % p.length;
  return p[idx];
}
...
function posterSVG(event, size = 'card') {
  ...
  const colorId = event.dbId || event.id;
  const [c1, c2] = posterColors(colorId);
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">
    ...
    <rect x="0" y="${h-5}" width="${w}" height="5" fill="rgba(255,255,255,0.15)"/>
      ...
      <div class="detail-info">
        ...
        <div class="detail-meta-card">
          ...
          <div class="detail-meta-row">
            ...
            <div><div class="meta-label">Time</div><div class="meta-value">${ev.time}</div><div class="meta-label" style="margin-top:3px">Doors open ${ev.doorsOpen}</div></div>
          </div>
          <div class="detail-meta-row">
        ...
        <div class="detail-extras">
          ...
          <div class="extra-panel" id="panel-photos">
            ...
            <div class="extra-panel-body"><p class="extra-empty">Photos available to attendees only.</p></div>
          </div>
        </div>
      </div>
    </div>
...
async function doRegister(eventId) {
  ...
  if (error && error.code === '23505') showToast("You're already registered!", 'error');
...
async function renderAccountPage() {
  ...
  if (authUser) {
    ...
    points = pts ? pts.reduce((sum, r) => sum + r.amount, 0) : 0;
  ...
  document.getElementById('account-content').innerHTML = `
    ...
    ${!authUser ? `<div style="background:#fff;border:1px solid #e0e0e0;border-radius:6px;padding:40px;text-align:center;margin-bottom:24px">
      <p style="font-size:15px;color:#555;margin-bottom:16px">Login to view your tickets, events, and referrals.</p>
    ...
    <div class="account-panel${activeAccountTab === 'tickets' ? ' active' : ''}" id="tab-tickets">
      ${tickets.length > 0 ? tickets.map(r => `
        <div class="ticket-card">
          <div class="ticket-poster" style="background:#111;display:flex;align-items:center;justify-content:center;font-size:32px">📅</div>
          <div class="ticket-body">
            <div class="ticket-title">${r.events?.title || 'Event'}</div>
            <div class="ticket-meta">${r.events?.start_at ? new Date(r.events.start_at).toLocaleDateString() : ''}<br>${r.events?.venue_name || ''}</div>
          </div>
        ...
        `<div class="empty-state">
          ...
          <p>No tickets yet. <a href="#" onclick="navigate('home');return false" style="color:#CC0000;font-weight:600">Browse events →</a></p>
        </div>`}
    </div>
    <div class="account-panel${activeAccountTab === 'events' ? ' active' : ''}" id="tab-events">
      ${authUser ? `<div style="margin-bottom:20px"><button class="btn-submit" onclick="navigate('create')">+ Create New Event</button></div>
      <div class="empty-state"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5"/></svg><p>No events created yet.</p></div>` :
    ...
    <div class="account-panel${activeAccountTab === 'referrals' ? ' active' : ''}" id="tab-referrals">
      <div class="referral-card">
        ...
        <p>Earn ZeePoints by attending events and referring friends. Redeem for prizes and discounts.</p>
        ${authUser ? `
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px">
          <div style="background:#f9f9f9;border:1px solid #e0e0e0;border-radius:4px;padding:14px;text-align:center">
            <div style="font-size:24px;font-weight:700;color:#1a1a1a">${points}</div>
            <div style="font-size:11px;color:#999;text-transform:uppercase;font-weight:600;letter-spacing:0.06em">Available</div>
          </div>
          <div style="background:#f9f9f9;border:1px solid #e0e0e0;border-radius:4px;padding:14px;text-align:center">
            <div style="font-size:24px;font-weight:700;color:#CC0000">${points}</div>
            <div style="font-size:11px;color:#999;text-transform:uppercase;font-weight:600;letter-spacing:0.06em">Lifetime</div>
          </div>
          <div style="background:#f9f9f9;border:1px solid #e0e0e0;border-radius:4px;padding:14px;text-align:center">
            <div style="font-size:24px;font-weight:700;color:#999">0</div>
            <div style="font-size:11px;color:#999;text-transform:uppercase;font-weight:600;letter-spacing:0.06em">Redeemed</div>
          </div>
        </div>
        <h4 style="margin-bottom:8px">🔗 Your Referral Link</h4>
        <p>Refer friends and earn 50 ZeePoints for each signup.</p>
        <div class="referral-link-box">
          <span>zeenle.com/ref/${(authProfile?.referral_code || authUser?.id?.slice(0,8))}</span>
          <button class="btn-copy" onclick="this.textContent='Copied!';navigator.clipboard?.writeText('zeenle.com/ref/${authProfile?.referral_code || ''}');setTimeout(()=>this.textContent='Copy',2000)">Copy</button>
        </div>` : `<div class="empty-state"><p>Login to view your points.</p></div>`}
      </div>
    </div>
  `;
}
...
function renderCreatePage() {
  document.getElementById('create-content').innerHTML = `
    ...
    <p class="form-page-sub">Fill in the details below. All events are reviewed before going live.</p>
    <div class="form-section">
      ...
      <div class="form-row single" style="margin-bottom:20px">
        <div class="form-field">
          ...
          <div class="upload-zone" onclick="document.getElementById('poster-input').click()">
            ...
            <p>Click to upload or drag & drop</p><span>PNG, JPG, MP4 — max 10MB</span>
            <input id="poster-input" type="file" accept="image/*,video/mp4" style="display:none" onchange="posterSelected(this)">
          </div>
        </div>
      </div>
      <div class="form-row" style="margin-bottom:20px">
        <div class="form-field"><label class="form-label">Event Title <span class="required">*</span></label><input id="ev-title" type="text" class="form-input" placeholder="e.g. Job Search Workshop"></div>
      ...
      <div class="form-row single" style="margin-bottom:20px">
        <div class="form-field"><label class="form-label">Description</label><textarea id="ev-desc" class="form-textarea" placeholder="Tell attendees what this event is about…"></textarea></div>
      </div>
      <div class="form-row triple" style="margin-bottom:20px">
        <div class="form-field"><label class="form-label">Start Date & Time <span class="required">*</span></label><input id="ev-start" type="datetime-local" class="form-input"></div>
        <div class="form-field"><label class="form-label">End Date & Time</label><input id="ev-end" type="datetime-local" class="form-input"></div>
        <div class="form-field"><label class="form-label">Doors Open</label><input id="ev-doors" type="datetime-local" class="form-input"></div>
      </div>
      <div class="form-row" style="margin-bottom:20px">
        <div class="form-field"><label class="form-label">Registration Method</label><div class="toggle-group"><button class="toggle-btn active" onclick="setFormToggle('registrationMethod','RSVP',this)">RSVP</button><button class="toggle-btn" onclick="setFormToggle('registrationMethod','Ticketing',this)">Ticketing</button></div></div>
        <div class="form-field"><label class="form-label">Currency</label><div class="toggle-group"><button class="toggle-btn active" onclick="setFormToggle('currency','CAD',this)">CAD</button><button class="toggle-btn" onclick="setFormToggle('currency','USD',this)">USD</button><button class="toggle-btn" onclick="setFormToggle('currency','EUR',this)">EUR</button></div></div>
      </div>
      <div class="form-row" style="margin-bottom:20px">
        <div class="form-field"><label class="form-label">Price</label><input id="ev-price" type="number" class="form-input" placeholder="0 for free" min="0" step="0.01"></div>
        <div class="form-field"><label class="form-label">Max Tickets</label><input id="ev-max" type="number" class="form-input" placeholder="0 = unlimited"></div>
      </div>
      <div class="form-row">
        <div class="form-field"><label class="form-label">Contact Email</label><input id="ev-email" type="email" class="form-input" placeholder="contact@example.com"></div>
        <div class="form-field"><label class="form-label">Contact Phone</label><input id="ev-phone" type="tel" class="form-input" placeholder="+1 (416) 000-0000"></div>
      </div>
    </div>
    <div class="form-section">
      ...
      <div class="form-row" style="margin-bottom:20px">
        <div class="form-field"><label class="form-label">Mode</label><div class="toggle-group"><button class="toggle-btn active" onclick="setFormToggle('addressMode','In Person',this)">In Person</button><button class="toggle-btn" onclick="setFormToggle('addressMode','Online',this)">Online</button><button class="toggle-btn" onclick="setFormToggle('addressMode','Hybrid',this)">Hybrid</button></div></div>
        <div class="form-field"><label class="form-label">Address Visible To</label><div class="toggle-group"><button class="toggle-btn active" onclick="setFormToggle('addressVisible','All',this)">All</button><button class="toggle-btn" onclick="setFormToggle('addressVisible','Approved Tickets',this)">Approved</button><button class="toggle-btn" onclick="setFormToggle('addressVisible','Email Only',this)">Email Only</button></div></div>
      </div>
      <div class="form-row" style="margin-bottom:20px">
        <div class="form-field"><label class="form-label">Venue Name</label><input id="ev-venue" type="text" class="form-input" placeholder="e.g. Welcome Centre"></div>
        <div class="form-field"><label class="form-label">Street Address</label><input id="ev-address" type="text" class="form-input" placeholder="123 Main St"></div>
      </div>
      <div class="form-row triple">
        <div class="form-field"><label class="form-label">City</label><input id="ev-city" type="text" class="form-input" placeholder="Toronto"></div>
        <div class="form-field"><label class="form-label">Province</label><input id="ev-province" type="text" class="form-input" placeholder="ON"></div>
        <div class="form-field"><label class="form-label">Postal Code</label><input id="ev-postal" type="text" class="form-input" placeholder="M1A 1A1"></div>
      </div>
    </div>
    <div class="form-section">
    ...
    <div class="form-section">
      ...
      <div class="lang-chips">${['English','French','Spanish','Persian','Hindi','Arabic','Mandarin','Cantonese','Portuguese','German','Dutch','Italian','Russian','Ukrainian'].map(l => `<button class="lang-chip${activeLangs.includes(l) ? ' active' : ''}" onclick="toggleLang('${l}',this)">${l}</button>`).join('')}</div>
    </div>
...
async function submitEvent() {
  ...
  const payload = {
    ...
    mode: addressMode,
    address_visible_to: addressVisible,
    ...
    tickets_at_door: ticketsAtDoor,
    allow_carpool: carpoolAllowed,
    status: 'published'
  };
...
function posterSelected(input) {
  ...
  zone.innerHTML = `<p style="color:#1a7a3a;font-weight:600">✓ ${file.name}</p><span>Click to change</span>`;
