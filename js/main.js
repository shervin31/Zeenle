/* ═══════════════════════════════════════════════════════
   Zeenle | main.js
   Full auth · Events · RSVP · Carpool · Location · Pages
═══════════════════════════════════════════════════════ */
...
const SAMPLES=[
  {id:1,title:"Job Search Workshop",category:"Workshop",date:"Saturday, June 6, 2026",time:"10:00 AM – 12:00 PM",doorsOpen:"9:45 AM",location:"Welcome Centre",address:"9325 Yonge St, Richmond Hill, ON",lat:43.8801,lng:-79.4400,speaker:"Behshad Sabah",desc:"An engaging workshop designed to inspire, inform, and empower professionals navigating today's competitive job market. Covers modern job search strategies, LinkedIn optimization, and interview techniques.",reg:"RSVP",price:"Free",mode:"In Person",lang:"Persian, English",carpool:true,tags:["Workshop","Career"],emoji:"💼"},
  {id:2,title:"Resume & Networking Workshop",category:"Workshop",date:"Saturday, June 6, 2026",time:"12:30 PM – 2:30 PM",doorsOpen:"12:15 PM",location:"Welcome Centre",address:"9325 Yonge St, Richmond Hill, ON",lat:43.8801,lng:-79.4400,speaker:"Behshad Sabah",desc:"Learn to craft a standout resume and build a professional network with real-world strategies used by top recruiters and hiring managers.",reg:"RSVP",price:"Free",mode:"In Person",lang:"Persian, English",carpool:true,tags:["Workshop","Career"],emoji:"📄"},
...
let userLat=null,userLng=null,locLabel='',sortByLoc=false;
...
function setBtnLoad(btn,txt,on=true){if(!btn)return;btn.textContent=txt;btn.disabled=on;}
function icon(d,s=15){return`<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="${s}" height="${s}"><path stroke-linecap="round" stroke-linejoin="round" d="${d}"/></svg>`;}
...
async function syncUser(user){
  authUser=user;
  const {data,error}=await sb.from('users').upsert({id:user.id,email:user.email,full_name:user.user_metadata?.full_name||user.user_metadata?.name||'',avatar_url:user.user_metadata?.avatar_url||''},{onConflict:'id'}).select().single();
...
async function sendOTP(){
  ...
  phoneTmp=num;const btn=g('ph-send');setBtnLoad(btn,'Sending…');
...
function niceErr(e){const m=e.message||'';if(m.includes('Invalid login'))return'Incorrect email or password.';if(m.includes('not confirmed'))return'Please confirm your email first.';if(m.includes('already registered'))return'Account already exists.';if(m.includes('not enabled')||m.includes('disabled'))return'Email login not enabled — enable it in Supabase Auth Providers.';return m;}
...
function requestLocation(){
  ...
  const btn=g('loc-main-btn');if(btn){btn.textContent='Locating…';btn.disabled=true;}
  navigator.geolocation.getCurrentPosition(async pos=>{
    userLat=pos.coords.latitude;userLng=pos.coords.longitude;
    try{
      const r=await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${userLat}&lon=${userLng}&format=json&zoom=14`);
      const d=await r.json();
      // Use neighborhood > suburb > district > city — more precise than just city
      locLabel=d.address?.neighbourhood||d.address?.suburb||d.address?.quarter||d.address?.district||d.address?.city_district||d.address?.city||d.address?.town||'your area';
    }catch(_){locLabel='your area';}
    sortByLoc=true;updateLocBar();loadEvents();
    toast(`Showing events near ${locLabel}`,'ok');
...
function clearLocation(){userLat=null;userLng=null;locLabel='';sortByLoc=false;updateLocBar();loadEvents();}
function updateLocBar(){
  const bar=g('loc-bar');if(!bar)return;
  if(sortByLoc&&locLabel){
    bar.innerHTML=`${icon('M15 10.5a3 3 0 11-6 0 3 3 0 016 0zM19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z',14)}<span class="loc-text">Showing events near <strong>${locLabel}</strong></span><span class="loc-badge">Near Me</span><button class="btn-loc-clear" onclick="clearLocation()">Clear</button>`;
  }else{
    bar.innerHTML=`${icon('M15 10.5a3 3 0 11-6 0 3 3 0 016 0zM19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z',14)}<span class="loc-text">Showing all events</span><button id="loc-main-btn" class="btn-loc" onclick="requestLocation()">Use my location</button>`;
  }
}
...
async function loadEvents(){
  try{
    ...
    }else if(EVENTS===EVENTS&&data?.length===0){
      // DB returned empty, keep samples
    }
  }catch(_){}
...
function posterColors(id){const P=[['#064E3B','#065F46'],['#1E1B4B','#312E81'],['#7F1D1D','#991B1B'],['#111827','#1F2937'],['#451A03','#78350F'],['#0C4A6E','#075985'],['#14532D','#166534'],['#4A044E','#701A75']];const i=typeof id==='number'?(id-1)%P.length:Math.abs((id||'').toString().charCodeAt(0)||0)%P.length;return P[i];}

function makePosterSVG(ev,forCard=true){
  ...
  return`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 400" style="width:100%;height:100%;display:block;position:absolute;inset:0">
    ...
    <rect y="348" width="300" height="52" fill="rgba(0,0,0,0.45)"/>
    <rect y="396" width="300" height="4" fill="#DC2626"/>
    <text x="16" y="32" font-family="system-ui,sans-serif" font-size="10" font-weight="700" fill="rgba(255,255,255,0.6)" letter-spacing="2.5">${cat}</text>
    ...
    ${lines.map((l,i)=>`<text x="16" y="${188+i*32}" font-family="Georgia,serif" font-size="22" font-weight="700" fill="#fff">${l}</text>`).join('')}
    ${ev.speaker?`<rect x="16" y="${188+lines.length*32+10}" width="3" height="18" fill="rgba(220,38,38,0.9)" rx="1.5"/><text x="25" y="${188+lines.length*32+24}" font-family="system-ui,sans-serif" font-size="12" fill="rgba(255,255,255,0.85)" font-weight="600">${ev.speaker}</text>`:''}
    <text x="16" y="367" font-family="system-ui,sans-serif" font-size="12" font-weight="600" fill="rgba(255,255,255,0.9)">${ev.date||''}</text>
    <text x="16" y="382" font-family="system-ui,sans-serif" font-size="11" fill="rgba(255,255,255,0.6)">${ev.time||''}</text>
...
function renderCards(list){
  ...
  if(!list?.length){grid.innerHTML='<div style="grid-column:1/-1;text-align:center;padding:60px;color:#9CA3AF;font-size:14px">No events found.</div>';return;}
  grid.innerHTML=list.map((ev,i)=>{
    const idx=EVENTS.findIndex(x=>String(x.id)===String(ev.id));
    const si=idx>=0?idx:i;
    const distBadge=ev.dist&&ev.dist<9999?`<span class="tag tag-dist">${icon('M15 10.5a3 3 0 11-6 0 3 3 0 016 0zM19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z',11)} ${ev.dist<1?(ev.dist*1000).toFixed(0)+'m':ev.dist.toFixed(1)+'km'}</span>`:'';
    const imgHTML=ev.poster_url
      ?`<div class="ecard-img-inner" style="background:#111"><img src="${ev.poster_url}" alt="${ev.title}" style="width:100%;height:100%;object-fit:cover;display:block;position:absolute;inset:0"></div>`
      :`<div class="ecard-img-inner">${makePosterSVG(ev,true)}</div>`;
    return`<div class="ecard fu" style="animation-delay:${i*.04}s" onclick="openDetail(${si})">
      ...
      <div class="ecard-foot">
        ...
        <button class="btn-view" onclick="event.stopPropagation();openDetail(${si})">Details →</button>
      </div>
    </div>`;
  }).join('');
}

function openDetail(idx){const ev=EVENTS[idx];if(!ev)return;go('detail',ev);}
...
async function renderDetail(){
  ...
  const posterHTML=ev.poster_url
    ...
    :`<div class="detail-poster-ph">${makePosterSVG(ev,false)}</div>`;

  g('detail-content').innerHTML=`
    <div class="breadcrumb"><a href="#" onclick="go('home');return false">← Back to Events</a></div>
    <div class="detail-grid">
      ...
      <div class="detail-side">
        ...
        <div class="meta-box">
          <div class="mrow"><div class="mrow-icon">${icon('M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5')}</div><div><div class="mrow-label">Date</div><div class="mrow-val">${ev.date}</div></div></div>
          <div class="mrow"><div class="mrow-icon">${icon('M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z')}</div><div><div class="mrow-label">Time</div><div class="mrow-val">${ev.time}</div><div class="mrow-sub">Doors open ${ev.doorsOpen}</div></div></div>
          <div class="mrow"><div class="mrow-icon">${icon('M15 10.5a3 3 0 11-6 0 3 3 0 016 0zM19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z')}</div><div><div class="mrow-label">Location</div><div class="mrow-val">${ev.location}</div><div class="mrow-sub">${ev.address}</div></div></div>
          ${ev.speaker?`<div class="mrow"><div class="mrow-icon">${icon('M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z')}</div><div><div class="mrow-label">Speaker</div><div class="mrow-val">${ev.speaker}</div></div></div>`:''}
          <div class="mrow"><div class="mrow-icon">${icon('M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z')}</div><div><div class="mrow-label">Price</div><div class="mrow-val">${ev.price}</div></div></div>
        </div>
        ...
        <div class="accordion">
          ...
          <div class="acc-item" id="acc-co">
            ...
            <div class="acc-body"><div class="acc-inner">
              ${coList.length?coList.map(o=>`<div class="carpool-entry"><strong>${o.users?.full_name||'Someone'}</strong> — ${o.seats_available} seat${o.seats_available>1?'s':''} from ${o.departure_location||'TBD'}<br><span style="font-size:11px;color:var(--text3)">${o.notes||''}</span></div>`).join(''):'<p style="color:var(--text3)">No carpool offers yet.</p>'}
              ${authUser?`<button class="btn-secondary" style="margin-top:10px;width:100%" onclick="openCarpoolModal('offer','${ev.dbId||ev.id}')">+ Offer a Ride</button>`:`<p style="font-size:12px;color:var(--text3);margin-top:8px"><a href="#" onclick="openModal()" style="color:var(--red);font-weight:600">Login</a> to offer a ride</p>`}
            </div></div>
          </div>
          <div class="acc-item" id="acc-cr">
            ...
            <div class="acc-body"><div class="acc-inner">
              ${crList.length?crList.map(r=>`<div class="carpool-entry"><strong>${r.users?.full_name||'Someone'}</strong> — from ${r.pickup_location||'TBD'}<br><span style="font-size:11px;color:var(--text3)">${r.notes||''}</span></div>`).join(''):'<p style="color:var(--text3)">No requests yet.</p>'}
              ${authUser?`<button class="btn-secondary" style="margin-top:10px;width:100%" onclick="openCarpoolModal('request','${ev.dbId||ev.id}')">+ Request a Ride</button>`:`<p style="font-size:12px;color:var(--text3);margin-top:8px"><a href="#" onclick="openModal()" style="color:var(--red);font-weight:600">Login</a> to request a ride</p>`}
            </div></div>
          </div>`:''}
          <div class="acc-item" id="acc-ph">
            ...
            <div class="acc-body"><div class="acc-inner" style="color:var(--text3)">Photos are available to attendees only.</div></div>
          </div>
        </div>
      </div>
    </div>`;
...
async function doRSVP(){
  ...
  const ev=selEv;if(!ev){toast('No event selected.','err');return;}
  // Need a valid UUID - only DB events have real UUIDs
  if(!ev.dbId||typeof ev.dbId==='number'){
    toast('This is a sample event. Create real events to RSVP.','err');return;
  }
  const{error}=await sb.from('registrations').insert({event_id:ev.dbId,user_id:authUser.id,status:'pending'});
...
function openCarpoolModal(type,evId){
  ...
  el.innerHTML=`<div class="cp-modal"><div class="cp-head"><h3>${isOffer?'🚗 Offer a Ride':'🙋 Request a Ride'}</h3><button class="drawer-x" onclick="document.getElementById('cp-overlay').remove()">✕</button></div><div class="cp-body"><div class="mfield"><label class="mlabel">${isOffer?'Departure':'Pickup'} Location <span style="color:var(--red)">*</span></label><input id="cp-loc" type="text" class="minput" placeholder="e.g. Yonge & Sheppard, Toronto"></div>${isOffer?`<div class="mfield"><label class="mlabel">Available Seats *</label><input id="cp-seats" type="number" class="minput" placeholder="3" min="1" max="8"></div><div class="mfield"><label class="mlabel">Departure Time</label><input id="cp-time" type="datetime-local" class="minput"></div>`:''}<div class="mfield"><label class="mlabel">Notes (optional)</label><textarea id="cp-notes" class="minput" style="min-height:70px;resize:vertical" placeholder="Any extra details…"></textarea></div><button class="btn-auth" onclick="submitCarpool('${type}','${evId}')">Submit</button></div></div>`;
...
async function submitCarpool(type,evId){
  ...
  const isOffer=type==='offer';
  const payload={event_id:evId,user_id:authUser.id,notes:v('cp-notes')};
  if(isOffer){
...
async function renderAccount(){
  ...
  g('account-content').innerHTML=`
    ...
    <div class="apanel${acctTab==='tickets'?' on':''}" id="ap-tickets">
      ${(tickets||[]).length?(tickets||[]).map(r=>`<div class="tcard"><div class="tcard-img">📅</div><div class="tcard-body"><div class="tcard-title">${r.events?.title||'Event'}</div><div class="tcard-meta">${r.events?.start_at?new Date(r.events.start_at).toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'}):''}${r.events?.venue_name?'<br>'+r.events.venue_name:''}</div></div><div class="tcard-side"><span class="status-pill ${r.status}">${r.status}</span></div></div>`).join(''):`<div class="empty">${icon('M16.5 6v.75m0 3v.75m0 3v.75m0 3V18m-9-5.25h5.25M7.5 15h3M3.375 5.25c-.621 0-1.125.504-1.125 1.125v3.026a2.999 2.999 0 010 5.198v3.026c0 .621.504 1.125 1.125 1.125h17.25c.621 0 1.125-.504 1.125-1.125v-3.026a3 3 0 010-5.198V6.375c0-.621-.504-1.125-1.125-1.125H3.375z',40)}<p>No tickets yet. <a href="#" onclick="go('home');return false" style="color:var(--red);font-weight:600">Browse events →</a></p></div>`}
    </div>
    <div class="apanel${acctTab==='events'?' on':''}" id="ap-events">
      ...
      ${(myEvs||[]).length?(myEvs||[]).map(e=>`<div class="tcard"><div class="tcard-img">📅</div><div class="tcard-body"><div class="tcard-title">${e.title}</div><div class="tcard-meta">${e.start_at?new Date(e.start_at).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}):''}<br><span style="font-size:11px;font-weight:700;text-transform:uppercase;color:${e.status==='published'?'var(--green)':'var(--text3)'}">${e.status}</span></div></div><div class="tcard-side"><button class="btn-view" style="font-size:12px" onclick="">Manage</button></div></div>`).join(''):`<div class="empty"><p>No events created yet.</p></div>`}
    </div>
...
function posterPicked(inp){const f=inp.files[0];if(!f)return;const z=inp.closest('.upload-area');z.innerHTML=`<p style="color:var(--green);font-weight:600;margin-bottom:4px">✓ ${f.name}</p><span>Click to change</span>`;z.onclick=()=>document.getElementById('f-poster').click();}
async function submitEvent(){
  ...
  else{toast('Event published! Redirecting to home…','ok');fLangs=[];EVENTS=[...SAMPLES];await loadEvents();go('home');}
}

/* ── ABOUT PAGE ── */
function renderAbout(){
  g('about-content').innerHTML=`
    <div class="page-hero">
      <h1>About Zeenle</h1>
      <p>A community-first events platform built to connect people through meaningful experiences.</p>
    </div>
    <div class="mission-card">
      <h2>Our Mission</h2>
      <p>Zeenle was created with a simple belief: communities grow stronger when people come together. We make it easy to discover local events — workshops, cultural screenings, wellness retreats, festivals — and equally easy to organize them. Whether you're attending your first event or hosting your hundredth, Zeenle is your home base for community connection in the Greater Toronto Area and beyond.</p>
    </div>
    <div class="values-grid">
      ...
      <div class="value-item"><div class="vi">🌱</div><h3>Local Impact</h3><p>We focus on real communities, real neighbourhoods, and real relationships.</p></div>
    </div>
    <div class="content-grid">
      <div class="content-card">
        ...
        <p>Today, Zeenle hosts workshops, screenings, retreats, and festivals across the GTA, bringing thousands of people together each month.</p>
      </div>
      <div class="content-card">
        <h2>${icon('M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z')} What We Offer</h2>
        <p>Free and paid event listing, RSVP and ticketing management, carpooling coordination, ZeePoints loyalty rewards, and a growing business directory for community vendors.</p>
        <p>All in one place, designed to be simple and fast.</p>
      </div>
    </div>
...
function renderContact(){
  g('contact-content').innerHTML=`
    <div class="page-hero">
      <h1>Contact Us</h1>
      <p>We'd love to hear from you. Reach out for partnerships, event support, or just to say hello.</p>
    </div>
    <div class="contact-layout">
      <div>
        <div class="contact-details">
          <h2>Get in Touch</h2>
          <div class="cd-row">${icon('M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z')}<span><strong>Phone</strong><br>416 300-0602</span></div>
          <div class="cd-row">${icon('M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75')}<span><strong>Email</strong><br>mrezair@zeenle.com</span></div>
          <div class="cd-row">${icon('M15 10.5a3 3 0 11-6 0 3 3 0 016 0zM19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z')}<span><strong>Location</strong><br>Greater Toronto Area, Ontario, Canada</span></div>
        </div>
        <div class="contact-form-card">
          <h2>Send a Message</h2>
          <div class="mfield"><label class="mlabel">Your Name</label><input type="text" class="minput" placeholder="Your full name"></div>
          ...
          <div class="mfield"><label class="mlabel">Message</label><textarea class="minput" style="min-height:110px;resize:vertical" placeholder="Tell us how we can help…"></textarea></div>
      ...
      <div class="ceo-card">
        <div class="ceo-photo">
          <div class="ceo-photo-placeholder">
            ${icon('M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z',64)}
            <p>CEO Photo</p>
          </div>
        </div>
        <div class="ceo-info">
          ...
          <p>With a passion for community and technology, Mohammad founded Zeenle to make it easier for people to discover and attend meaningful local events. His vision is a platform where every community — regardless of language or background — has a home.</p>
        </div>
      </div>
    </div>`;
}
