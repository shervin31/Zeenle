/* ═══════════════════════════════════════════════════════════════
   ...
   Auth · Events · RSVP · Carpool · Location · Create Event
═══════════════════════════════════════════════════════════════ */
...
/* ─── Sample events (fallback when DB empty) ─────────────────── */
const SAMPLES = [
...
async function sendOTP() {
  const num = v('ph-number'); if (!num) { toast('Enter a phone number.','err'); return; }
  phoneTmp = num;
  const btn=g('ph-send-btn'); setBtnLoad(btn,'Sending…');
...
async function verifyOTP() {
  ...
  const btn=g('ph-verify-btn'); setBtnLoad(btn,'Verifying…');
...
function renderPhone() {
  ...
  if (phoneStep==='number') {
    el.innerHTML=`<div class="mfield"><label class="mlabel">Phone Number</label><input id="ph-number" type="tel" class="minput" placeholder="+1 (416) 000-0000" autocomplete="tel"><div style="font-size:11px;color:var(--text-3);margin-top:4px">We'll send a verification code via SMS.</div></div><button id="ph-send-btn" class="btn-submit" onclick="sendOTP()">Send Code →</button>`;
  } else if (phoneStep==='otp') {
    el.innerHTML=`<p style="font-size:13px;color:var(--text-2);margin-bottom:14px">Code sent to <strong>${phoneTmp}</strong></p><div class="mfield"><label class="mlabel">Verification Code</label><input id="ph-otp" type="text" class="minput" placeholder="000000" maxlength="6" autocomplete="one-time-code" style="letter-spacing:5px;font-size:20px;text-align:center"></div><button id="ph-verify-btn" class="btn-submit" onclick="verifyOTP()">Verify →</button><button onclick="phoneStep='number';renderPhone()" style="width:100%;padding:8px;font-size:12px;color:var(--text-3);background:none;border:none;cursor:pointer;margin-top:4px">← Different number</button>`;
  } else {
    el.innerHTML=`<p style="font-size:13px;color:var(--text-2);margin-bottom:14px">Almost done — what's your name?</p><div class="mfield"><label class="mlabel">Full Name</label><input id="ph-name" type="text" class="minput" placeholder="Your name" autocomplete="name"></div><button class="btn-submit" onclick="savePhoneName()">Finish →</button>`;
  }
}

async function doLogout() {
  await sb.auth.signOut();
  closeDropdown(); closeDrawer();
  toast('Logged out.');
}

/* ─── Password checks ────────────────────────────────────────── */
function pwOk(v) { return v.length>=8 && /[A-Z]/.test(v) && /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(v); }
...
function niceErr(e) {
  ...
  if(m.includes('not enabled')) return 'This login method is not set up yet.';
  return m;
}
...
function refreshAuthUI() {
  ...
  if(av) av.textContent = (profile?.full_name||authUser?.email||'?').split(' ').map(n=>n[0]).join('').slice(0,2).toUpperCase();
  const un=g('nav-username');
  if(un) un.textContent = (profile?.full_name||authUser?.email||'').split(' ')[0];
  const da=g('drawer-auth');
  if(da) da.innerHTML = in_
    ? `<a href="#" onclick="go('account');return false"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"/></svg>My Account</a><div class="dsep"></div><a href="#" onclick="doLogout();return false" style="color:var(--accent)"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9"/></svg>Log Out</a>`
    : `<a href="#" onclick="openModal('login');closeDrawer();return false"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9"/></svg>Login / Sign Up</a>`;
}
  ...
  navigator.geolocation.getCurrentPosition(
    async pos => {
      ...
      try {
        ...
        userCity = data.address?.city || data.address?.town || data.address?.village || data.address?.suburb || 'your area';
      } catch(_) { userCity = 'your area'; }
      sortByLoc = true;
      updateLocationBar();
      loadEvents();
      toast(`Showing events near ${userCity}`,'ok');
    },
    err => {
      if (err.code===1) toast('Location access denied.','err');
...
function clearLocation() {
  userLat=null; userLng=null; userCity=''; sortByLoc=false;
  updateLocationBar();
  loadEvents();
}
...
function updateLocationBar() {
  ...
  if (sortByLoc && userCity) {
    bar.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"/></svg><span class="loc-text">Showing events near <strong>${userCity}</strong></span><span class="loc-badge">Near Me</span><button class="loc-btn" onclick="clearLocation()">Clear</button>`;
  } else {
    bar.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"/></svg><span class="loc-text">Showing all events</span><button class="loc-btn" onclick="requestLocation()">Use my location →</button>`;
  }
}
...
async function loadEvents() {
  try {
    ...
    if (!error && data?.length) {
      EVENTS = data.map(e => ({
        ...
        speaker:e.speaker||null, desc:e.description||'',
  ...
  if (sortByLoc && userLat && userLng) {
    list = list.map(e => ({
      ...e,
      dist: (e.lat&&e.lng) ? distKm(userLat,userLng,e.lat,e.lng) : 9999
...
function posterColors(id) {
  const P=[['#1A3A2A','#2D6A4A'],['#1a1a2e','#16213e'],['#7B0D1E','#C0392B'],['#1C1C1C','#3A3A3A'],['#3D2B1F','#7D5A3C'],['#0D1B2A','#1B4F72'],['#0B3D0B','#1A7A1A'],['#2D1B69','#5B2D8E']];
  const i = typeof id==='number'?(id-1)%P.length:Math.abs((id||'').charCodeAt(0)||0)%P.length;
  return P[i];
}

function makePoster(ev, size='card') {
  const w=400, h=size==='detail'?560:520;
  const [c1,c2]=posterColors(ev.dbId||ev.id);
  const cat=(ev.category||'EVENT').toUpperCase();
  const title=(ev.title||'');
  const titleLines = title.length>20 ? [title.slice(0,20), title.slice(20,40)] : [title];
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" style="width:100%;height:100%;display:block">
    <defs>
      <linearGradient id="pg${ev.id}" x1="0" y1="0" x2="0.6" y2="1">
        <stop offset="0%" stop-color="${c1}"/>
        <stop offset="100%" stop-color="${c2}"/>
      </linearGradient>
    </defs>
    <rect width="${w}" height="${h}" fill="url(#pg${ev.id})"/>
    <rect y="${h-5}" width="${w}" height="5" fill="rgba(212,0,26,0.8)"/>
    <rect x="0" y="0" width="${w}" height="56" fill="rgba(0,0,0,0.25)"/>
    <text x="20" y="34" font-family="system-ui,sans-serif" font-size="11" font-weight="700" fill="rgba(255,255,255,0.6)" letter-spacing="3">${cat}</text>
    <text x="${w-20}" y="34" font-family="system-ui,sans-serif" font-size="11" font-weight="600" fill="rgba(255,255,255,0.5)" text-anchor="end">${ev.mode||'In Person'}</text>
    <text x="20" y="130" font-family="system-ui,sans-serif" font-size="72" fill="rgba(255,255,255,0.9)">${ev.emoji||'📅'}</text>
    ${titleLines.map((l,i)=>`<text x="20" y="${220+i*38}" font-family="Georgia,serif" font-size="28" font-weight="700" fill="#fff">${l}</text>`).join('')}
    ${ev.speaker?`<rect x="20" y="${titleLines.length===2?310:280}" width="3" height="22" fill="rgba(212,0,26,0.9)" rx="1.5"/><text x="30" y="${titleLines.length===2?327:297}" font-family="system-ui,sans-serif" font-size="13" fill="rgba(255,255,255,0.9)" font-weight="600">${ev.speaker}</text>`:''}
    <rect x="0" y="${h-90}" width="${w}" height="90" fill="rgba(0,0,0,0.35)"/>
    <text x="20" y="${h-60}" font-family="system-ui,sans-serif" font-size="13" font-weight="600" fill="rgba(255,255,255,0.95)">${ev.date||''}</text>
    <text x="20" y="${h-40}" font-family="system-ui,sans-serif" font-size="12" fill="rgba(255,255,255,0.7)">${ev.time||''}</text>
    <text x="20" y="${h-18}" font-family="system-ui,sans-serif" font-size="11" fill="rgba(255,255,255,0.55)">${ev.location||''}</text>
  </svg>`;
}
...
function renderCards(list) {
  ...
  if(!list||!list.length) { grid.innerHTML='<div style="grid-column:1/-1;text-align:center;padding:60px;color:#999">No events found.</div>'; return; }
  grid.innerHTML=list.map((ev,i)=>{
    const evIdx = EVENTS.indexOf(ev)!==-1 ? EVENTS.indexOf(ev) : i;
    const distLabel = ev.dist && ev.dist<9999 ? `<span style="font-size:11px;color:var(--accent);font-weight:600">📍 ${ev.dist.toFixed(1)} km away</span>` : '';
    return `<div class="ecard fu" style="animation-delay:${i*.04}s" onclick="go('detail',EVENTS[${evIdx}]||SAMPLES[${i}])">
      ...
      <div class="ecard-foot">
        ...
        <button class="btn-view" onclick="event.stopPropagation();go('detail',EVENTS[${evIdx}]||SAMPLES[${i}])">View Details →</button>
      </div>
    </div>`;
  }).join('');
}
...
async function renderDetail() {
  ...
  // Load carpool data from DB
  let carpoolOffers=[], carpoolRequests=[];
  if(ev.dbId) {
    ...
    carpoolOffers = co||[]; carpoolRequests = cr||[];
  }
    ...
    <div class="detail-grid">
      ...
      <div class="detail-side">
        ...
        <div class="detail-meta-box">
          ${dmr('date',ev.date,'Date','M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5')}
          ${dmrSub(ev.time,'Time',`Doors open ${ev.doorsOpen}`,'M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z')}
          ${dmrSub(ev.location,'Location',ev.address,'M15 10.5a3 3 0 11-6 0 3 3 0 016 0zM19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z')}
          ${ev.speaker?dmr('person',ev.speaker,'Speaker','M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z'):''}
          ${dmr('price',ev.price,'Price','M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z')}
        </div>
        ...
        <div class="accordion">
          ...
          <div class="acc-item" id="acc-co">
            <div class="acc-head" onclick="toggleAcc('co')">
              <span>🚗 Carpool Offers <span style="font-size:11px;color:var(--text-3);font-weight:400">(${carpoolOffers.length})</span></span>
            ...
            <div class="acc-body"><div class="acc-inner">
              ${carpoolOffers.length ? carpoolOffers.map(o=>`<div style="padding:8px 0;border-bottom:1px solid var(--border);last:border:none"><strong>${o.users?.full_name||'Someone'}</strong> — ${o.seats_available} seat${o.seats_available>1?'s':''} from ${o.departure_location||'TBD'}<br><span style="font-size:11px;color:var(--text-3)">${o.notes||''}</span></div>`).join('') : '<p>No carpool offers yet.</p>'}
              ${authUser?`<button class="btn-share" style="margin-top:10px;width:100%" onclick="openCarpoolModal('offer','${ev.dbId||ev.id}')">+ Offer a Ride</button>`:'<p style="font-size:12px;color:var(--text-3);margin-top:8px"><a href="#" onclick="openModal()" style="color:var(--accent)">Login</a> to offer a ride</p>'}
            </div></div>
          </div>
          <div class="acc-item" id="acc-cr">
            <div class="acc-head" onclick="toggleAcc('cr')">
              <span>🙋 Carpool Requests <span style="font-size:11px;color:var(--text-3);font-weight:400">(${carpoolRequests.length})</span></span>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5"/></svg>
            </div>
            <div class="acc-body"><div class="acc-inner">
              ${carpoolRequests.length ? carpoolRequests.map(r=>`<div style="padding:8px 0;border-bottom:1px solid var(--border)"><strong>${r.users?.full_name||'Someone'}</strong> — from ${r.pickup_location||'TBD'}<br><span style="font-size:11px;color:var(--text-3)">${r.notes||''}</span></div>`).join('') : '<p>No requests yet.</p>'}
              ${authUser?`<button class="btn-share" style="margin-top:10px;width:100%" onclick="openCarpoolModal('request','${ev.dbId||ev.id}')">+ Request a Ride</button>`:'<p style="font-size:12px;color:var(--text-3);margin-top:8px"><a href="#" onclick="openModal()" style="color:var(--accent)">Login</a> to request a ride</p>'}
            </div></div>
          </div>`:''}
          <div class="acc-item" id="acc-ph">
            <div class="acc-head" onclick="toggleAcc('ph')"><span>📸 Event Photos</span><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5"/></svg></div>
            <div class="acc-body"><div class="acc-inner">Photos are available to attendees only.</div></div>
          </div>
        </div>
      </div>
    </div>`;
...
function dmr(icon, val, label, path) {
  return `<div class="dmr"><div class="dmr-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="${path}"/></svg></div><div><div class="dmr-label">${label}</div><div class="dmr-value">${val}</div></div></div>`;
}
function dmrSub(val, label, sub, path) {
  return `<div class="dmr"><div class="dmr-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="${path}"/></svg></div><div><div class="dmr-label">${label}</div><div class="dmr-value">${val}</div><div class="dmr-sub">${sub}</div></div></div>`;
}
...
function shareEv() {
  if(navigator.share) navigator.share({title:selEv?.title, url:location.href});
  else { navigator.clipboard?.writeText(location.href); toast('Link copied!','ok'); }
}

/* ─── Carpool Modal ──────────────────────────────────────────── */
let carpoolType='offer', carpoolEventId='';
function openCarpoolModal(type, eventId) {
  carpoolType=type; carpoolEventId=eventId;
  const isOffer = type==='offer';
  const modal = document.createElement('div');
  modal.id='carpool-modal-overlay';
  modal.style.cssText='position:fixed;inset:0;z-index:600;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;padding:16px';
  modal.innerHTML=`<div style="background:#fff;border-radius:6px;padding:0;width:100%;max-width:380px;box-shadow:0 20px 60px rgba(0,0,0,0.2);overflow:hidden">
    <div style="padding:18px 20px;border-bottom:1px solid #e0e0e0;display:flex;align-items:center;justify-content:space-between">
      <h3 style="font-size:15px;font-weight:700">${isOffer?'Offer a Ride':'Request a Ride'}</h3>
      <button onclick="document.getElementById('carpool-modal-overlay').remove()" style="width:28px;height:28px;border-radius:50%;border:1px solid #e0e0e0;background:#fff;cursor:pointer;font-size:14px;color:#999">✕</button>
    </div>
    <div style="padding:20px">
      <div class="mfield"><label class="mlabel">${isOffer?'Departure':'Pickup'} Location</label><input id="cp-location" type="text" class="minput" placeholder="e.g. Yonge & Sheppard, Toronto"></div>
      ${isOffer?`<div class="mfield"><label class="mlabel">Available Seats</label><input id="cp-seats" type="number" class="minput" placeholder="e.g. 3" min="1" max="8"></div>`:''}
      ${isOffer?`<div class="mfield"><label class="mlabel">Departure Time</label><input id="cp-time" type="datetime-local" class="minput"></div>`:''}
      ...
      <button class="btn-submit" onclick="submitCarpool()">Submit</button>
    </div>
  </div>`;
...
async function submitCarpool() {
  const loc = document.getElementById('cp-location')?.value?.trim();
  if(!loc){toast('Enter a location.','err');return;}
  const isOffer=carpoolType==='offer';
  let payload = { event_id:carpoolEventId, user_id:authUser.id, notes:document.getElementById('cp-notes')?.value||'' };
  if(isOffer) {
    payload.departure_location=loc;
    payload.seats_available=parseInt(document.getElementById('cp-seats')?.value||1);
    const t=document.getElementById('cp-time')?.value;
  ...
  document.getElementById('carpool-modal-overlay')?.remove();
  toast(`Carpool ${isOffer?'offer':'request'} submitted!`,'ok');
  renderDetail(); // Refresh detail page
}
...
async function renderAccount() {
  ...
  tickets=rData||[];
  const { data:myEvents } = await sb.from('events').select('*').eq('created_by',authUser.id).order('created_at',{ascending:false});
  ...
  g('account-content').innerHTML=`
    ...
    <div class="acct-tabs">
      ...
      <div class="atab${acctTab==='events'?' on':''}" onclick="switchAcctTab('events')">My Events (${myEvents?.length||0})</div>
    ...
    <div class="apanel${acctTab==='events'?' on':''}" id="ap-events">
      ...
      ${myEvents?.length ? myEvents.map(e=>`<div class="tcard" style="cursor:pointer" onclick="go('detail',{id:'${e.id}',dbId:'${e.id}',title:'${e.title}',category:'Event',date:'',time:'',doorsOpen:'',location:e.venue_name||'',address:e.address||'',desc:e.description||'',reg:e.registration_method||'RSVP',price:e.price>0?'$'+e.price+' '+e.currency:'Free',mode:e.mode||'In Person',lang:'',carpool:e.allow_carpool,tags:['Event'],emoji:'📅'})"><div class="tcard-poster">📅</div><div class="tcard-body"><div class="tcard-title">${e.title}</div><div class="tcard-meta">${e.start_at?new Date(e.start_at).toLocaleDateString():''}<br><span style="font-size:11px;font-weight:600;color:${e.status==='published'?'var(--green)':'var(--text-3)'};text-transform:uppercase">${e.status}</span></div></div><div class="tcard-side" style="padding:14px;border-left:1px solid var(--border)"><button class="btn-view" style="white-space:nowrap">Edit</button></div></div>`).join('') : `<div class="empty"><p>No events created yet.</p></div>`}
    </div>
    <div class="apanel${acctTab==='referrals'?' on':''}" id="ap-referrals">
      <div class="ref-card">
        ...
        <div class="ref-link">
          <span id="ref-url">zeenle.com/ref/${profile?.referral_code||authUser.id.slice(0,8)}</span>
          <button class="btn-copy" onclick="navigator.clipboard?.writeText(g('ref-url').textContent);this.textContent='Copied!';setTimeout(()=>this.textContent='Copy',2000)">Copy</button>
        </div>
      </div>
    </div>`;
}
...
function renderCreate() {
  g('create-content').innerHTML=`
    ...
    <p class="form-page-sub">Fill in the details below to publish your event on Zeenle.</p>
    <div class="fsec">
    ...
    <div class="fsec">
      ...
      <div class="frow"><div class="ff"><label class="flabel">Venue Name</label><input id="f-venue" type="text" class="finput" placeholder="Venue name"></div><div class="ff"><label class="flabel">Street Address</label><input id="f-addr" type="text" class="finput" placeholder="123 Main St"></div></div>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Ask for location after a short delay (non-intrusive)
  setTimeout(() => {
    if(!userLat && !localStorage.getItem('loc-dismissed')) {
      const bar = g('loc-bar');
      if(bar && !sortByLoc) {
        // Already showing the "Use my location" button, no need for a popup
      }
    }
  }, 1500);
});